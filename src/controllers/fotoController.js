...
const uploadArquivo = (tipo) => async (req, res) => {
    try {
        ...
        if (!user) return res.status(404).json({ error: 'Registro não encontrado.' });
        if (!req.file) return res.status(400).json({ error: 'Nenhum arquivo enviado.' });

        if (user[tipo]) await deletarStorage(user[tipo]);
        user[tipo] = await uploadStorage(id, req.file);
...
const buscarArquivo = (tipo) => async (req, res) => {
    try {
        ...
        const user = await UsuarioModel.buscarPorId(parseInt(id));
        if (!user) return res.status(404).json({ error: 'Registro não encontrado.' });
        if (!user[tipo]) return res.status(404).json({ error: `Nenhum ${tipo} cadastrado.` });

        return res.status(200).json({ url: user[tipo] });
    } catch (error) {
...
const deletarArquivo = (tipo) => async (req, res) => {
    try {
        ...
        const user = await UsuarioModel.buscarPorId(parseInt(id));
        if (!user) return res.status(404).json({ error: 'Registro não encontrado.' });
        if (!user[tipo]) return res.status(404).json({ error: `Nenhum ${tipo} para remover.` });

        await deletarStorage(user[tipo]);
        user[tipo] = null;
